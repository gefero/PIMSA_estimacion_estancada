# Estimación de la cantidad de trabajadores por cuenta propia de baja calificación urbanos

## Introducción

En el auto-empleo se confunden posiciones sociales diversas: pequeños 
empresarios con alguna capacidad de acumulación, profesionales independientes 
con honorarios, pequeños comerciantes y productores con alguna posibilidad de 
ahorro y todo tipo de proletarios con trabajos irregulares. Esto es lo que 
posibilita las dimensiones de comparación puedan fácilmente, en un instante 
orientarse hacia el capital, extendiéndose a los empleadores, e incluso en 
escalas que pueden culminar en empresas con un número muy considerable de
empleados, y al siguiente hacia el trabajo asalariado, ampliándose hacia sus 
capas más bajas por pobres, por menos calificadas o por subordinadas en forma 
encubierta por el capital. 
“Exitosos” y “fracasados”, en una línea que debe hacer numerosas abstracciones 
para llegar a ese continuo, según en qué dirección se quiera prolongarla. El 
trabajo por cuenta propia funciona así como el vector de la informalidad que 
permite ver en el vendedor ambulante tanto a un gran comerciante en potencia 
como a un trabajador sin derechos.
Hay allí o bien un capital latente a ser desencadenado o bien un asalariado 
encadenado  a ser reconocido, pero no superpoblación relativa. En el trabajador 
por cuenta propia (y por extensión, en la informalidad) quedan depositados 
todos las promesas que se espera que el capitalismo cumpla: para los reformistas, 
un lugar reconocido en la división del trabajo para todos los trabajadores, 
para los liberales, la liberación del empresario que pretende que cada uno de 
ellos esconde. Nada que un ajuste institucional no pueda resolver.
La pobreza, la imposibilidad de fijar horarios propios y la dependencia de 
algún cliente dominante nos podrían estar indicando la presencia de algunos 
rasgos proletarios entre esta población aparentemente independiente. Pero 
incluso estos elementos podrían funcionar tan solo como un mínimo: el hecho de 
que el ingreso de nuestro vendedor ambulante provenga de numerosos clientes de poca monta cada uno, que él decida por sí mismo extender su horario por el escaso resultado de su jornada o que eventualmente el trabajo sin descanso le rinda lo suficiente para estar al día, nada de esto hace cambiar su posición social.
Pero llegados a este punto, lo mismo puede pasarle al comerciante que abre su negocio temprano y lo cierra tarde para atender a clientes múltiples y variados que le permitan asegurarse una remuneración que no lo haga pasar por pobre. No se trata solo de que las herramientas estadísticas parecen insuficientes para distinguir al pequeño comerciante del vendedor ambulante, sino que las imprecisiones en los rasgos no son meramente instrumentales, corresponden a una concepción teórica a partir de la cual está construido un instrumento que detecta más similitudes que diferencias en los rasgos de quienes acceden a medios de vida en base a la pequeña propiedad respecto de aquellos que lo logran irregularmente a falta de ella. El trabajo por cuenta propia funciona como emplasto por sobre las diferencias entre pequeños propietarios y población obrera sobrante.  Su perdurabilidad como categoría estadística se sostiene en parte en lo que renuncia a conocer.
No podremos aquí superar las limitaciones que nos impone para una comparación internacional. Sin embargo, en función de lo que venimos desarrollando buscaremos otra aproximación, consistente en utilizar como principal indicador el nivel de calificación de la ocupación.
Según la Clasificación Internacional Uniforme de Ocupaciones (CIUO-08) utilizada por OIT (2012) las ocupaciones son discriminadas en las siguientes categorías, las cuales a su vez son agrupadas en la plataforma ILOSTAT en tres grandes grupos:

| Grandes grupos                                                            | Nivel general de calificación           |
|---------------------------------------------------------------------------|-----------------------------------------|
| 1 Gerentes                                                                |                   Alta                  |
| 2 Profesionales                                                           |                   Alta                  |
| 3 Técnicos y profesionales asociados                                      |                   Alta                  |
| 4 Personal de apoyo administrativo                                        |                  Media                  |
| 5 Trabajadores de servicios y ventas                                      |                  Media                  |
| 6 Trabajadores cualificados de la agricultura, la silvicultura y la pesca |                  Media                  |
| 7 Artesanos y trabajadores de oficios afines                              |                  Media                  |
| 8 Operadores de instalaciones y máquinas y montadores                     |                  Media                  |
| 9 Ocupaciones elementales                                                 |                   Baja                  |
| 0 Ocupaciones militares                                                   | Se distribuyen entre Alta, Media y Baja |
Fuente: OIT (s. f.).
 
Se suele distinguir entre los trabajadores por cuenta propia a aquellos de calificación alta (grupos 1, 2 y 3) a los cuales se suele asimilar con los empleadores. Esto tiene sentido si se pretende identificar a la capa superior del auto-empleo.  A los fines del problema que abordamos la distinción deja indiscriminada a la capa inferior, la que se agrupa como “ocupaciones elementales” (grupo 9), que es la que aquí nos interesa.
Precisamente este último grupo comprende a las ocupaciones consideradas de baja calificación. Se trata de aquellas que implican la realización de tareas simples y rutinarias, que pueden requerir el uso de herramientas manuales básicas y un esfuerzo o resistencia física considerable y requieren  escasa o nula necesidad de formación formal o un breve período de aprendizaje en el puesto de trabajo.  Entre ellas se incluyen: limpieza, reposición de suministros y mantenimiento básico; lavado de autos y ventanas;  ayuda en cocinas y tareas simples de preparación de alimentos;  entrega de mensajes o mercancías;  carga y manipulación de equipaje y fletes; abastecimiento de máquinas expendedoras y lectura de parquímetros;  recolección y clasificación de residuos; barrido de calles; tareas simples agropecuarias, pesqueras, de caza o captura; tareas simples ligadas a minería, construcción y manufactura; selección, empaquetado y desempaquetado manual de productos y reposición de mostradores, servicios callejeros; transporte de personas y bienes en vehículos a pedal, guiados a mano o de tracción animal.
En la última revisión del clasificador se señala que el grupo de ocupaciones elementales fue especialmente reformulado para volver más visibles en los datos internacionales a aquellas ocupaciones que predominan en el empleo informal (OIT, 2012: 36-37). A pesar de ello, varias modificaciones introducidas respecto de versiones anteriores hacen que una parte de esas ocupaciones quede agrupada fuera de dicho conjunto. Así, una porción de los vendedores callejeros se desprende de las ocupaciones elementales y pasa a integrarse dentro de “trabajadores de los servicios y ventas” (gran grupo 5), como ocurre con los vendedores puerta a puerta y los vendedores de alimentos para consumo inmediato que no sean de preparación rápida y que tengan contacto directo con el público. También fue trasladado desde las ocupaciones elementales a ese grupo el grueso del personal de seguridad, quedando solo los vigiladores, serenos y guardapuertas. Aunque sin cambios respecto de versiones anteriores, se despliegan subgrupos para cuidadoras y niñeras y, se advierte que las trabajadoras sexuales se encuentran bajo el discreto subgrupo de trabajadores de “servicios personales no clasificados en otros grupos”. Finalmente, se indica que se ha decidido visibilizar ocupaciones informales basadas en la artesanía del vidrio, la cerámica, los textiles, la paja, la piedra, la madera y el cuero, pero dentro de “artesanos y trabajadores de oficio afines” (gran grupo 7).
Sin advertencias similares, no hay forma de discriminar tampoco  al interior de otras ocupaciones que ocupan un lugar recurrente en la literatura sobre el trabajo informal: feriantes de puesto fijo en mercados (grupo 5), talleristas de pequeñas manufacturas (grupo 7) y choferes de vehículos a motor particulares (grupo 8), en cada uno de los cuales se confunden propietarios y no propietarios.
En su afán de distinguir algún oficio simple o una habilidad sencilla respecto de la tarea repetitiva y sin calificación, el clasificador termina dando vuelos a un sistema de exacerbación de las mínimas diferencias. El mensajero puede ser el mismo pero si deja su bicicleta para manejar una motocicleta, pasará de “elemental” a “operador de maquinaria” y en ese grupo se encontrará asimilado al chofer de un automóvil. Este sistema crece en distorsión cuanto más se concentran distintas tareas en una sola o muy pocas personas. Bajo estas diferencias un mismo puesto callejero puede generar distintos perfiles según se hagan primar las tareas de preparación de comida, de venta o de atención al público.  Quien atiende la parrilla en un puesto de choripanes quedará en un gran grupo diferente del compadre que expende y cobra al cliente, y este último, a su vez, quedará dentro del mismo que el comerciante que mira con desprecio a ambos desde la puerta de su negocio (cfr. OIT, 2012: 246, sobre la cantidad de aclaraciones necesarias para desambiguar estas categorías).
Claro que de tener la información sobre cada subgrupo algunas podrían ser reagrupadas, pero a fines de la gran estadística la información se resume a los grandes grupos y de allí la limitación en la disponibilidad de datos. Con todo, el grupo de ocupaciones elementales queda compuesto por una amplia variedad de subgrupos,

91 — Limpiadores y asistentes

- 911 — Limpiadores y asistentes domésticos, de hoteles y oficinas
        - 9111 — Limpiadores y asistentes domésticos
        - 9112 — Limpiadores y asistentes de oficinas, hoteles y otros establecimientos
- 912 — Lavanderos a mano, limpiadores de vehículos, ventanas y otros limpiadores manuales
        - 9121 — Lavanderos y planchadores a mano
        - 9122 — Limpiadores de vehículos
        - 9123 — Limpiadores de ventanas
        - 9129 — Otros trabajadores de limpieza

92 — Peones agropecuarios, forestales y pesqueros
- 921 — Peones agropecuarios, forestales y pesqueros
        - 9211 — Peones de explotaciones agrícolas
        - 9212 — Peones de explotaciones ganaderas
        - 9213 — Peones de explotaciones mixtas agrícolas y ganaderas
        - 9214 — Peones de jardinería y horticultura
        - 9215 — Peones forestales
        - 9216 — Peones de la pesca y la acuicultura

93 — Peones de la minería, la construcción, la manufactura y el transporte
- 931 — Peones de la minería y la construcción
        - 9311 — Peones de la minería y las canteras
        - 9312 — Peones de obras de ingeniería civil
        - 9313 — Peones de la construcción de edificios
- 932 — Peones de la manufactura
        - 9321 — Empaquetadores manuales
        - 9329 — Peones de la manufactura no clasificados en otros grupos
- 933 — Peones del transporte y almacenamiento
        - 9331 — Conductores de vehículos de tracción humana y pedal
        -  9332 — Conductores de vehículos y maquinaria de tracción animal
        -  9333 — Cargadores
        -  9334 — Repositores de estanterías

94 — Ayudantes de preparación de alimentos
- 941 — Ayudantes de preparación de alimentos
        - 9411 — Preparadores de comidas rápidas
        - 9412 — Ayudantes de cocina

95 — Vendedores ambulantes y trabajadores de servicios callejeros afines
- 951 — Trabajadores de servicios callejeros y afines
        - 9510 — Trabajadores de servicios callejeros y afines
- 952 — Vendedores ambulantes (excepto de alimentos)
        - 9520 — Vendedores ambulantes (excepto de alimentos)

96 — Recolectores de desechos y otros trabajadores elementales
- 961 — Recolectores de desechos
        - 9611 — Recolectores de basura y material reciclable
        - 9612 — Clasificadores de desechos
        -  9613 — Barrenderos y afines
- 962 — Otros trabajadores elementales
        -  9621 — Mensajeros, repartidores de paquetes y maleteros
        -  9622 — Trabajadores ocasionales / changarines de arreglos básicos
        -  9623 — Lectores de medidores y recolectores de máquinas expendedoras
        -  9624 — Recolectores de agua y leña
        -  9629 — Trabajadores elementales no clasificados en otros grupos

Estas ocupaciones bien pueden ser realizadas también en forma asalariada,  donde la forma de aproximación supone sus propias complejidades estadísticas (Donaire y de Souza, 2025), pero allí parecería improbable de que se dude de su carácter proletario. Que estas ocupaciones se realicen como trabajador por cuenta propia o familiar está más bien dando cuenta de la irregularidad e intermitencia más que la independencia de una unidad económica. Indican la ausencia de medios de trabajo y de vida propios, o en todo caso, la presencia de medios de trabajo y de vida que sólo pueden ser considerados tales bajo un modo de trabajo miserable: desde el carro de un cartonero, pasando por la bicicleta de un repartidor, las chucherías de un mantero, el escurridor de un limpiavidrios, el trapo de un cuidacoche, hasta llegar a las estampitas de un mendigo. Son ocupaciones que no exigen una calificación especial, lo que sumado al tipo de medios utilizados, las constituyen como un rebusque y un refugio de aquella población proletaria expropiada pero sobrante para el capital.
La aproximación a través de la delimitación de este grupo supone una serie de decisiones:
La primera refiere a los trabajadores por cuenta propia y familiares en la actividad agrícola. Si bien el clasificador de ocupaciones incluye dentro del gran grupo de “ocupaciones elementales” a una categoría específica para “peones agropecuarios, forestales y pesqueros” (subgrupo 92), la  presentación de la  información disponible no permite discriminarlos. Por otra parte, como hemos señalado, la forma particular de desarrollo capitalista en el agro hace que la principal forma de manifestación de la superpoblación allí sea bajo la modalidad latente. Esto no impide que la industrialización del agro genere sus propias formas de superpoblación estancada (Rosati y Chazarreta, 2025). Sin embargo, en los términos planteados por el CIUO-08, la mayoría de los agricultores quedan agrupados como “trabajadores cualificados de la agricultura, la silvicultura y la pesca” (gran grupo 6), donde se incluyen tanto aquellos “orientados al mercado” como “de subsistencia”. A pesar de que allí se confunden campesinos, agricultores familiares, pequeños productores, etc., en términos de su posición en el movimiento de la estructura social, ocupan un lugar diferente de los pequeños productores industriales y comerciales. Por eso, decidimos considerar a la agricultura en forma diferenciada como rama, e introducir la distinción entre ocupaciones para las restantes actividades “no agrícolas”. Esto a su vez enlaza con las preocupaciones originarias de las tesis de la “informalidad” ligada al problema de población relativamente excedente urbana.
La segunda decisión, tal vez la más importante para el análisis: es posible que una parte de la superpoblación relativa que se presenta como TCP/TF no aparezca entre las “ocupaciones elementales”. Esto sucede principalmente en ocupaciones que quedan clasificadas como de calificación media. Si aplicáramos las categorías actuales a la población analizada por Hart, buena parte sería considerada de baja calificación: vendedores ambulantes, cargadores, lavanderas, lustrabotas, vaciadores de letrinas, barrenderos, sirvientes domésticos, ayudantes, peones, buscavidas y serenos, pero otras, corresponderían a la calificación media: comerciantes e intermediarios, prestamistas, puesteros, barberos, cocineros, artesanos, destiladores, molineros, zapateros, sastres, reparadores, instaladores, taxistas, choferes, prostitutas y adivinos  -esto sin considerar la larga lista de los que serían indetectables: vendedores de droga, proxenetas, contrabandistas, chantajistas, carteristas, ladrones y gariteros, timadores, estafadores y mendigos-. Como hemos visto hoy podríamos agregar a niñeras, cuidadoras, talleristas, ensambladores, mensajeros, choferes, etc. pero también, vendedores telefónicos, mozos, promotores, cargadores de datos, etc. Todos ellos quedan comprendidos en una misma categoría ampliamente poblada de pequeños comerciantes, propietarios de tiendas, prestadores y productores, de las cuales, o bien, el clasificador las hace indistinguibles o, si permiten distinguirlas, dicho grado de desagregación no se encuentra disponible.  Aquí no es posible conocer ni la proporción de poblaciones  distintas de propietarios y no propietarios  encubiertas bajo la misma categoría ni estimar en qué volumen existe una población que oscila en el límite entre ambas. Pero si en el ejercicio del comercio o un arte específico puede existir el margen de la confusión, en las ocupaciones consideradas elementales,  la ausencia de oficio y la presencia de medios rudimentarios para llevarlas adelante, ponen de manifiesto que su trabajo simple debe estar de alguna forma organizado, subordinado, pautado por otros o dependiente en extremo de la ayuda  (cuando no del deshecho) de terceros.
Finalmente, la prioridad dada en la identificación de las ocupaciones de calificación baja deja en el grupo a las ocupaciones de calificación alta en el mismo conjunto con las de calificación media. Nuevamente, la forma de disposición de datos dificultan la aproximación complementaria necesaria.  Acá me sigue haciendo ruido porque el grupo de alta calificación es minoritario pero incluso más importante que el de los de baja calificación. En todo caso, la restricción a  las ocupaciones elementales nos permitirá establecer un parámetro mínimo, que no sobreestime ni fuerce la hipótesis de la superpoblación en estas categorías, como ocurre cuando por el contrario, se distingue a la categoría de mayor calificación frente al resto.
Las limitaciones en la disposición refieren a que la plataforma ILOSTAT no presenta en una misma tabla los datos de la población ocupada según situación de la ocupación (donde podemos identificar a trabajadores por cuenta propia y familiares), de la población según grupo ocupacional (para ubicar las elementales) y rama de actividad (para distinguir las no agrícolas). La información se presenta en tres tablas bivariadas:
- Población ocupada, según sexo, situación en el empleo y grupo ocupacional (miles). Anual.
 Id: EMP_TEMP_SEX_STE_OCU_NB_A
- Población ocupada, según sexo, situación en el empleo y rama de actividad económica (miles). Anual.
 Id: EMP_TEMP_SEX_STE_ECO_NB_A
- Población ocupada, según rama de actividad económica y grupo ocupacional (miles). Anual.Id: EMP_TEMP_ECO_OCU_NB_A

Para estimar la cantidad de trabajadores por cuenta propia de baja calificación del sector no agraria,
se parte de las tres tablas bivaradas mencionadas y se busca estimar a partir de las mismas la distriución conjunta:
la tabla trivariada que combine/cruce rama de actividad, situación en el empleo y calificación de la ocupación.

Para esto se usa el método de Iterative Proportional Fitting (IPF): en R, con el paquete `mipfp` (función `Ipfp`);
en Python, con una implementación propia (`src/ipf_utils.py`) usada para validar de forma independiente los
resultados de R (ver "Pruebas realizadas").

## Código

Pipeline de estimación en R (orden de ejecución):
- `./src/011_preproc_estimacion_tcp_estancada.R` — descarga de ILOSTAT (`Rilostat`) y agregación de las tres
  tablas bivariadas (crudas y agregadas por país). **Corrige un bug de agregación**: hay que sumar las
  categorías finas dentro de cada año y recién después promediar entre años (`group_by(..., time) %>%
  summarise(sum) %>% ... %>% summarise(mean)`); un `mean()` directo sobre datos ya des-agregados por año
  dividía "No_agro" (~5 categorías ECO agrupadas) por su cantidad de subcomponentes, inflando el margen
  "Agro" en ~25 puntos porcentuales en la estimación final.
- `./src/012_estimacion_tcp_estancada.R` — corre el IPF (`mipfp::Ipfp`) por país sobre las tres tablas
  agregadas y arma la tabla trivariada de salida.
- `./src/013_tcp_estancada_tabla_final.R` — arma la tabla final con indicadores por país (marginales +
  celda de interés) y el join con la clasificación de países.
- `./src/014_tcp_estancada_analysis.R` — análisis sustantivo (clusters, regiones, etc.) sobre la tabla final.

Scripts de validación:
- `./src/015_analisis_pruebas_ipf.R` — análisis de las dos pruebas de validación (EPH e IPUMS), en
  R/tidyverse (`dplyr`/`tidyr`/`purrr`/`readr`/`stringr`/`ggplot2`). Mismos parámetros que la versión Python
  (`--estimacion <csv>`, `--sufijo <str>`). **Camino usado actualmente** para no depender de Python; el
  mapeo país→iso3c usa `./data/outputs/country_classification.csv` en vez de `pycountry` (dos excepciones
  manuales: Iran→"Iran, Islamic Rep.", Palestine→"West Bank and Gaza"). Requiere `r-base-core` +
  `r-cran-{dplyr,tidyr,purrr,readr,stringr,ggplot2,glue,tibble}` (instalables vía apt con
  `--no-install-recommends`, sin necesitar CRAN directo). Nota: correrlo con `LC_ALL=C.UTF-8` si el locale
  del sistema es `C`/`POSIX` — si no, los acentos y símbolos (ρ) se corrompen en las figuras.
- `./src/015_analisis_pruebas_ipf.py` — misma lógica en Python (pandas/numpy/matplotlib/pycountry), no
  depende de `Rilostat` ni de rutas locales fuera del repo. Se mantiene como implementación alternativa/histórica.
- `./src/016_pipeline_corregido.py` + `./src/ipf_utils.py` — reimplementación en Python del pipeline
  011→012→013 (agregación corregida + IPF propio), usada para (a) tener una estimación reproducible en
  entornos sin R/`Rilostat`, y (b) validar de forma independiente los resultados de `mipfp::Ipfp` en R.

## Rutas de datos (relativas a la raíz del repo)

- `./data/raw_data/` — tablas bivariadas crudas de ILOSTAT, una fila por país-año-categoría
  (`calif_rama.csv`, `catocup_calif.csv`, `catocup_rama.csv`). Salida de `011`, insumo de `013` y de `016`.
- `./data/estimacion/` — tablas agregadas por país (`calif_rama_agg.csv`, `catocup_calif_agg.csv`,
  `catocup_rama_agg.csv`), `country_intersect.csv` (intersección de países entre las tres tablas, 159
  países), la estimación IPF trivariada (`AAAAMMDD_estimacion_tcp_final_v2.csv`, salida de `012` — actualizar
  `ESTIMACION_PATH` en `013` a la versión vigente en cada corrida) y `tabla_tcps_final_sums.csv` (**tabla
  final con los resultados desagregados por país**, salida de `013`, 181 países).
- `./data/outputs/` — `country_classification.csv` (clasificación de países: región, ingreso, cluster PIMSA,
  etc., usada en el join final de `013`).
- `./data/test_ipf/` — salidas de las pruebas de validación (ver abajo).
- `./data/estimacion_tcp_final_corregida.csv` / `./data/tabla_tcps_final_sums_corregida.csv` — estimación y
  tabla final calculadas con el pipeline Python (`016`), equivalentes a las de R para comparación directa.
- `./reports/` — informes de análisis (`analisis_pruebas_ipf.md`, `testeo_python_vs_r_20260824.md`) y sus
  figuras (`./reports/figs/`).

## Pruebas realizadas

- **Estimación TCP baja calificación no agro EPH-IPF (Argentina)**: a partir de la EPH se reconstruyeron las
  tres tablas bivariadas para Argentina, se estimó el IPF y se compararon los resultados contra los datos
  observados. Valida que el método (no los insumos de ILOSTAT) funciona correctamente: error máximo por celda
  de 0,02 pp. Resultados en `./data/eph_ipf_comparacion_agg_test.csv` y `./data/test_ipf/`.

- **Estimación TCP baja calificación no agro IPUMS-IPF (mundo)**: se compara la estimación en base a las
  tablas de la OIT con las muestras censales de IPUMS (https://international.ipums.org/international/), 47
  países. Insumo: `./data/ipums_ifp_v2_tcp_by_calif.csv`. Este cruce fue el que permitió detectar el bug de
  agregación de `011` (margen "Agro" implícito en la estimación publicada +24,8 pp por encima de IPUMS antes
  de la corrección; −2,6 pp después). Resultados en `./data/test_ipf/` (`comp_raking_ipums_full*.csv`,
  `metricas_por_celda*.csv`, `celda_clave_paises*.csv`) y en `./reports/analisis_pruebas_ipf.md`.
  **Re-corrida al día (2026-08-24, sufijo `_v3`)** contra el `raw_data` vigente (commit `55a01f8`) con
  `src/015_analisis_pruebas_ipf.R`, usando `./data/estimacion/20260824_estimacion_tcp_final_v2.csv` (R) como
  insumo. Resultados prácticamente idénticos a la corrida anterior (`_v2`, sobre `raw_data` pre-refresh):
  MAE 12 celdas 2,15 pp, Pearson 0,928, sesgo margen agro −2,5 pp — sin señal de que el bug de agregación
  haya reaparecido. Detalle y trazabilidad completa en `./reports/analisis_pruebas_ipf.md` §9.

- **Validación cruzada R vs. Python del IPF (`mipfp::Ipfp` vs. implementación propia)**: se corrió el IPF de
  forma independiente en R (paquete `mipfp` real) y en Python (`016`) sobre los mismos insumos agregados
  (159 países), para descartar que hubiera una diferencia de algoritmo entre ambas implementaciones. Resultado:
  correlación 0,9955, sesgo medio ≈0, mediana del error absoluto 0,004 pp en la celda de interés; una cola de
  ~10 países (Senegal, Burkina Faso, Guinea-Bissau, entre los mayores) con diferencias de hasta 1,75 pp,
  atribuible a inconsistencia entre los márgenes bivariados de entrada de esos países (límite conocido del
  método, no bug de implementación). Detalle completo, metodología y tabla país por país en
  `./reports/testeo_python_vs_r_20260824.md`; salidas en `./data/test_ipf/estimacion_R_mipfp_full_independiente.csv`,
  `./data/test_ipf/comparacion_python_vs_R_completa.csv` y `./data/test_ipf/celda_interes_comparacion_final.csv`.

## Cambios realizados (changelog resumido)

1. Corregido el bug de agregación en `011` (sum por año → mean entre años) y bugs menores en `012`/`013`
   (alineación de dimensiones del IPF, ruta de la estimación a usar, dedupe del join de clasificación de
   países) y en `102`/`103` (año censal IPUMS conservado, ruta de `source()`, armonización de etiquetas).
2. Corregido un bug de 27 países duplicados en `tabla_tcps_final_sums.csv`: la fila del indicador clave
   armaba `ref_area.label` con `countrycode::countrycode()` en vez de tomarlo de las tablas ILOSTAT nativas,
   lo que partía cada país afectado en dos filas al hacer `pivot_wider()`.
3. Reorganización de carpetas de datos: `./data/estimacion_estancada/` (no versionada) → `./data/raw_data/` +
   `./data/estimacion/` + `./data/outputs/` (versionadas), con las rutas de `011`/`012`/`013` actualizadas
   en consecuencia.
4. Agregado el pipeline paralelo en Python (`016` + `ipf_utils.py`) y el script de validación parametrizado
   (`015`), para poder correr y validar la estimación sin depender de R/`Rilostat`.

## Tu tarea

**Resuelto (2026-08-24): re-corrida de la prueba IPUMS-IPF (mundo) contra los datos vigentes.** El
`raw_data` de ILOSTAT se había refrescado (commit `55a01f8`, revisiones de la serie 2009-2019) después de
que se generaran los resultados de validación IPUMS que figuraban en `./reports/analisis_pruebas_ipf.md` §9
(commit `9bf0624`). Se resolvió así:

1. Se confirmó que `./data/estimacion/20260824_estimacion_tcp_final_v2.csv` (salida real de `mipfp::Ipfp`
   en R, commiteada junto con el refresh de `raw_data` en `55a01f8`) ya estaba generada sobre los datos
   vigentes — no hizo falta re-ejecutar `011`→`012`→`013`.
2. Se portó `./src/015_analisis_pruebas_ipf.py` a R/tidyverse (`./src/015_analisis_pruebas_ipf.R`, mismos
   `--estimacion`/`--sufijo`) para no depender de Python, y se corrió con
   `--estimacion ./data/estimacion/20260824_estimacion_tcp_final_v2.csv --sufijo _v3`.
3. Se actualizó `./reports/analisis_pruebas_ipf.md` §9 con los resultados `_v3` (tabla comparativa de tres
   columnas: publicada con bug / corregida sobre `raw_data` pre-refresh / corregida sobre `raw_data`
   vigente), dejando registro explícito de commits y archivos usados.

(La prueba EPH-IPF Argentina no requirió re-correrse: no depende de `raw_data` de ILOSTAT; se re-generó
igual, sin cambios, como subproducto de correr `015`.)

Próximos pasos sugeridos: re-correr `014` (análisis por clusters/regiones) sobre
`tabla_tcps_final_sums.csv` ya corregida, y estimar un factor de corrección del sesgo de método del IPF
(subestimación estructural de la celda de interés, ≈−0,33 pp en el self-test IPUMS `_v3`).
